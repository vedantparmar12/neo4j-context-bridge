...
const QueryGraphSchema = {
  cypher: z
    ...
    .describe("Cypher query to execute (e.g., MATCH (n) RETURN n LIMIT 10)"),
};
...
export function registerNeo4jTools(server: McpServer, env: Env, props: Props) {
        ...
        return await withNeo4j(
          ...
          async (session) => {
            ...
            for (const label of labels) {
              ...
              labelCounts[label] = countResult.records[0].get('count');
            }
  ...
  // Tool 2: Query graph (read-only)
  server.tool(
    "queryGraph",
    "Execute a read-only Cypher query against the Neo4j graph database. This tool only allows MATCH, RETURN, and other read operations.",
    QueryGraphSchema,
    async ({ cypher }) => {
      try {
        ...
        if (isWriteOperation(cypher)) {
          return createErrorResponse(
            "Write operations are not allowed with this tool. Use the `executeCypher` tool if you have write permissions."
          );
        }
        ...
        return createErrorResponse(`Graph query error: ${formatNeo4jError(error)}`);
      }
    }
  );

  // Tool 3: Execute Cypher (write operations) - privileged users only
  if (ALLOWED_USERNAMES.has(props.login)) {
    server.tool(
      "executeCypher",
      "Execute any Cypher statement against the Neo4j graph database, including CREATE, MERGE, DELETE, and other write operations. **USE WITH CAUTION** - this can modify or delete data.",
      ExecuteCypherSchema,
      async ({ cypher }) => {
        try {
          return await withNeo4j(
  ...
  // Tool 5: Setup Vector Index (privileged users only)
  if (ALLOWED_USERNAMES.has(props.login)) {
    server.tool(
      "setupVectorIndex",
      "Setup vector index for semantic search on Context nodes. This enables fast similarity search on embeddings.",
      SetupVectorIndexSchema,
      async () => {
        try {
          return await withNeo4j(
            env.NEO4J_URI,
            ...
            async (session) => {
              try {
                ...
                // Note: Neo4j 5.x requires vector index support
                const result = await session.run(
                  `
                  CREATE VECTOR INDEX contextEmbeddings IF NOT EXISTS
                  FOR (c:Context)
                  ON c.embedding
                  OPTIONS {
                    indexConfig: {
                ...
                return createSuccessResponse(
                  ...
                  {
                    message: "Vector index 'contextEmbeddings' created for Context nodes",
                    dimensions: 384,
                    similarity: "cosine",
                    note: "You can now use vector similarity queries on Context embeddings"
                  }
                );
              } catch (error: any) {
                // Check if it's because vector indexes aren't supported
                if (error.message?.includes('Unknown function') || error.message?.includes('vector')) {
                  // Fallback message for older Neo4j versions
                  return createSuccessResponse(
                    "Vector index not supported",
                    {
                      message: "Your Neo4j version doesn't support vector indexes",
                      fallback: "The system will use in-memory similarity calculations instead",
                      recommendation: "For better performance, consider upgrading to Neo4j 5.13+ or using Neo4j AuraDB"
                    }
                  );
                }
                throw error;
          ...
          console.error('setupVectorIndex error:', error);
          return createErrorResponse(
            `Failed to setup vector index: ${formatNeo4jError(error)}`
          );
        }
      }
    );
